...
db.serialize(function() {
  db.run("CREATE TABLE IF NOT EXISTS users (user_id text unique)");
...
bot.on('inline_query', function (msg) {
  ...
  db.serialize(function() {
    ...
    db.get("SELECT user_id FROM users WHERE user_id=?",userData._id, function(err, row) {


      if(row==undefined){
        stmt = db.prepare("INSERT INTO users VALUES (?)");
        stmt.run(msg.from.id);
        stmt.finalize();
